 ...
 *   - HP 75%/50%때만 발악(패링)이 뜬다(25%때는 더 이상 안 뜸) — 1페이즈의 "죽으면 뜨는
 *     최후의 한 번"과 달리, 성공/실패해도 전투가 끝나지 않고(실패하면 목숨만 깎이고)
 *     계속 이어지다가, HP가 0이 되는 순간 그게 곧 최종 승리다.
 *   - 발악(패링)은 1페이즈와 완전히 동일한 사양이다(반복된 "너무 빠르다" 피드백으로
 *     통일) — 링 2.2초, 판정창 1.4초, 한 번만 좁혀진다.
 */
export const BOSS_PHASE2 = {
  ...
  pattern2JudgeMs: 220,
  pattern2DeathPenalty: 3,
  // 25% 때는 더 이상 뜨지 않는다 — 75%/50%때만.
  checkpointThresholds: [0.75, 0.5],
