 ...
 *   - HP 50%에 도달하면 "뒤바뀐 현실" 구간에 들어간다 — 암전과 예고 대사 뒤 화면이
 *     거꾸로 뒤집히고, 패턴1의 위험구역(빨간 경고)을 터치해야 데미지가 들어가며
 *     반대로 빈 구역을 터치하면 목숨이 깎이는 반전 판정이 적용된다. HP 40%에서
 *     원래대로 돌아온다.
 */
export const BOSS_PHASE2 = {
  ...
  pattern2JudgeMs: 220,
  pattern2DeathPenalty: 3,
  // 25%·50% 때는 더 이상 뜨지 않는다(50%는 "뒤바뀐 현실" 구간과 겹쳐서 뺐다) — 75%때만.
